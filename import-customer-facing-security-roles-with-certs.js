...
async function importCustomerFacingSecurityRolesWithCerts() {
  console.log('Starting Customer Facing Security Roles import with certifications...');

  try {
    // Read the Customer Facing Security Roles track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_17_Customer_Facing_Security_Roles_with_Certs.xlsx');
    ...
    // Find the Customer Facing Security Roles track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Customer%' AND name ILIKE '%Facing%' AND name ILIKE '%Security%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Customer Facing Security Roles track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Customer Facing Security Roles', 'Provide cybersecurity services, consulting, and support directly to clients and customers in external-facing positions']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Customer Facing Security Roles track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for customer facing roles
    ...
    console.log('\nCustomer Facing Security Roles import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Customer Facing Security Roles track:', error);
  } finally {
    await pool.end();
  }
}

importCustomerFacingSecurityRolesWithCerts();