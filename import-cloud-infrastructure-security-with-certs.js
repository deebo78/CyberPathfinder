...
async function importCloudInfrastructureSecurityWithCerts() {
  console.log('Starting Cloud and Infrastructure Security import with certifications...');

  try {
    // Read the Cloud and Infrastructure Security track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_8_Cloud_and_Infrastructure_Security_with_Certs.xlsx');
    ...
    // Find the Cloud and Infrastructure Security track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Cloud%' AND name ILIKE '%Infrastructure%' AND name ILIKE '%Security%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Cloud and Infrastructure Security track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Cloud and Infrastructure Security', 'Secure cloud environments and infrastructure systems with focus on scalable security solutions']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Cloud and Infrastructure Security track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for cloud and infrastructure
      ...
      if (certName.includes('gcp')) certMap.set('gcp', cert.id);
      if (certName.includes('google cloud')) certMap.set('google cloud', cert.id);
    ...
    console.log('\nCloud and Infrastructure Security import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Cloud and Infrastructure Security track:', error);
  } finally {
    await pool.end();
  }
}

importCloudInfrastructureSecurityWithCerts();