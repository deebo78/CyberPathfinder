...
async function importExecutiveLeadershipCISOWithCerts() {
  console.log('Starting Executive Leadership CISO Track import with certifications...');

  try {
    // Read the Executive Leadership CISO Track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_13_Executive_Leadership_CISO_Track_with_Certs.xlsx');
    ...
    // Find the Executive Leadership CISO track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Executive%' OR name ILIKE '%CISO%' OR name ILIKE '%Leadership%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Executive Leadership CISO track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Executive Leadership CISO Track', 'Executive cybersecurity leadership roles focused on strategic planning, governance, and organizational security management']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Executive Leadership CISO track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for executive leadership
      ...
      if (certName.includes('ccsp')) certMap.set('ccsp', cert.id);
    ...
    console.log('\nExecutive Leadership CISO Track import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Executive Leadership CISO Track:', error);
  } finally {
    await pool.end();
  }
}

importExecutiveLeadershipCISOWithCerts();