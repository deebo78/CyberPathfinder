...
async function importThreatIntelligenceWithCerts() {
  console.log('Starting Threat Intelligence import with certifications...');

  try {
    // Read the Threat Intelligence track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_18_Threat_Intelligence_with_Certs.xlsx');
    ...
    // Find the Threat Intelligence track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Threat%' AND name ILIKE '%Intelligence%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Threat Intelligence track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Threat Intelligence', 'Collect, analyze, and disseminate threat intelligence to support proactive cybersecurity defenses and strategic decision-making']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Threat Intelligence track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for threat intelligence
      ...
      if (certName.includes('gcti')) certMap.set('gcti', cert.id);
    ...
    console.log('\nThreat Intelligence import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Threat Intelligence track:', error);
  } finally {
    await pool.end();
  }
}

importThreatIntelligenceWithCerts();