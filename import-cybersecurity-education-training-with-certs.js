...
async function importCybersecurityEducationTrainingWithCerts() {
  console.log('Starting Cybersecurity Education & Training import with certifications...');

  try {
    // Read the Cybersecurity Education & Training track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_12_Cybersecurity_Education_Training_with_Certs.xlsx');
    ...
    // Find the Cybersecurity Education & Training track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Education%' AND name ILIKE '%Training%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Cybersecurity Education & Training track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Cybersecurity Education & Training', 'Develop and deliver cybersecurity training programs, curricula, and educational content for workforce development']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Cybersecurity Education & Training track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for education and training
    ...
    console.log('\nCybersecurity Education & Training import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Cybersecurity Education & Training track:', error);
  } finally {
    await pool.end();
  }
}

importCybersecurityEducationTrainingWithCerts();