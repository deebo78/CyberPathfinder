...
async function importSOCOperationsWithCerts() {
  try {
    console.log('Starting SOC Operations import with certifications...');
    ...
    // Find SOC Operations track (should be ID 31 based on previous data)
    const socTrack = await db.select().from(careerTracks).where(eq(careerTracks.name, 'SOC Operations')).limit(1);
    
    if (!socTrack.length) {
      console.error('SOC Operations track not found');
      return;
    }
    
    const trackId = socTrack[0].id;
    console.log(`Found SOC Operations track with ID: ${trackId}`);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const certificationsList = row[3];
          ...
          if (matchingCert) {
            ...
            if (!existingMapping.length) {
              await db.insert(careerLevelCertifications).values({
                ...
                notes: `Recommended for ${careerLevelName} in SOC Operations`
              });
    ...
    console.log('\nSOC Operations import with certifications completed successfully!');
    
  } catch (error) {
    console.error('Error importing SOC Operations with certifications:', error);
  } finally {
    await pool.end();
  }
}

importSOCOperationsWithCerts();