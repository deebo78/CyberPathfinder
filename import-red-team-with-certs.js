...
async function importRedTeamWithCerts() {
  try {
    console.log('Starting Red Team Operations import with certifications...');
    
    // Read the Excel file
    const workbook = XLSX.readFile('./attached_assets/Track_2_Red_Team_Operations_with_Certs.xlsx');
    
    // Get all sheet names and process the first one
    ...
    // Find Red Team Operations track
    const redTeamTrack = await db.select().from(careerTracks).where(eq(careerTracks.name, 'Red Team Operations')).limit(1);
    
    if (!redTeamTrack.length) {
      console.error('Red Team Operations track not found');
      return;
    }
    
    const trackId = redTeamTrack[0].id;
    console.log(`Found Red Team Operations track with ID: ${trackId}`);
    
    // Process each row
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      const careerLevelName = row[0]; // 'Entry-Level', 'Mid-Level', etc.
      const jobTitle = row[1];
      const niceWorkRoleCode = row[2];
      const notes = row[3];
      const certificationsList = row[4]; // Relevant Certifications column
      
      console.log(`\nProcessing: ${careerLevelName} - ${jobTitle}`);
      
      // Find or get the career level
      ...
      // Update or create the career position
      ...
      if (certificationsList && certificationsList.trim()) {
        ...
        for (let j = 0; j < certNames.length; j++) {
          ...
          // Try to find matching certification by name (fuzzy matching)
          ...
          if (matchingCert) {
            // Check if this certification is already mapped to this level
            ...
            if (!existingMapping.length) {
              await db.insert(careerLevelCertifications).values({
                ...
                priority: j + 1, // Order based on position in list
                notes: `Recommended for ${careerLevelName} in Red Team Operations`
              });
    ...
    console.log('\nRed Team Operations import with certifications completed successfully!');
    
  } catch (error) {
    console.error('Error importing Red Team Operations with certifications:', error);
  } finally {
    await pool.end();
  }
}

importRedTeamWithCerts();