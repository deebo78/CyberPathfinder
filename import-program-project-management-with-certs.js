...
async function importProgramProjectManagementWithCerts() {
  console.log('Starting Program and Project Management import with certifications...');

  try {
    // Read the Program and Project Management track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_14_Program_and_Project_Management_with_Certs.xlsx');
    ...
    // Find the Program and Project Management track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Program%' AND name ILIKE '%Project%' AND name ILIKE '%Management%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Program and Project Management track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Program and Project Management', 'Lead cybersecurity projects and programs, managing resources, timelines, and deliverables for security initiatives']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Program and Project Management track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for program and project management
    ...
    console.log('\nProgram and Project Management import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Program and Project Management track:', error);
  } finally {
    await pool.end();
  }
}

importProgramProjectManagementWithCerts();