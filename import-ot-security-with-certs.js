...
async function importOTSecurityWithCerts() {
  console.log('Starting OT Security import with certifications...');

  try {
    // Read the OT Security track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_10_OT_Security_with_Certs.xlsx');
    ...
    // Find the OT Security track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%OT%' OR name ILIKE '%Operational Technology%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('OT Security track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['OT Security', 'Secure operational technology and industrial control systems in critical infrastructure environments']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found OT Security track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for OT Security
    ...
    console.log('\nOT Security import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing OT Security track:', error);
  } finally {
    await pool.end();
  }
}

importOTSecurityWithCerts();