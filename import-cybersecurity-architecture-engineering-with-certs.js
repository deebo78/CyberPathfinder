...
async function importCybersecurityArchitectureEngineeringWithCerts() {
  console.log('Starting Cybersecurity Architecture Engineering import with certifications...');

  try {
    // Read the Cybersecurity Architecture Engineering track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_6_Cybersecurity_Architecture_Engineering_with_Certs.xlsx');
    ...
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Architecture%' AND name ILIKE '%Engineering%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Cybersecurity Architecture Engineering track not found, creating it...');
      ...
      console.log(`Found Cybersecurity Architecture Engineering track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for architecture and engineering
      ...
      if (certName.includes('sabsa')) certMap.set('sabsa', cert.id);
      if (certName.includes('togaf')) certMap.set('togaf', cert.id);
      if (certName.includes('aws')) certMap.set('aws', cert.id);
      if (certName.includes('azure')) certMap.set('azure', cert.id);
      if (certName.includes('cisco')) certMap.set('cisco', cert.id);
      if (certName.includes('cissp')) certMap.set('cissp', cert.id);
    ...
    console.log('\nCybersecurity Architecture Engineering import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Cybersecurity Architecture Engineering track:', error);
  } finally {
    await pool.end();
  }
}

importCybersecurityArchitectureEngineeringWithCerts();