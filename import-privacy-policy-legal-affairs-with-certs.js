...
async function importPrivacyPolicyLegalAffairsWithCerts() {
  console.log('Starting Privacy, Policy & Legal Affairs import with certifications...');

  try {
    // Read the Privacy, Policy & Legal Affairs track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_19_Privacy_Policy_Legal_Affairs_with_Certs.xlsx');
    ...
    // Find the Privacy, Policy & Legal Affairs track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Privacy%' OR name ILIKE '%Policy%' OR name ILIKE '%Legal%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Privacy, Policy & Legal Affairs track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Privacy, Policy & Legal Affairs', 'Develop and implement privacy policies, legal compliance frameworks, and cybersecurity governance programs']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Privacy, Policy & Legal Affairs track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for privacy, policy and legal
      ...
      if (certName.includes('cism')) certMap.set('cism', cert.id);
      if (certName.includes('cisa')) certMap.set('cisa', cert.id);
      if (certName.includes('crisc')) certMap.set('crisc', cert.id);
      if (certName.includes('cgeit')) certMap.set('cgeit', cert.id);
      if (certName.includes('cipp')) certMap.set('cipp', cert.id);
      if (certName.includes('cipm')) certMap.set('cipm', cert.id);
      if (certName.includes('cipt')) certMap.set('cipt', cert.id);
    });
    ...
    console.log('\nPrivacy, Policy & Legal Affairs import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Privacy, Policy & Legal Affairs track:', error);
  } finally {
    await pool.end();
  }
}

importPrivacyPolicyLegalAffairsWithCerts();