...
- **Map Vacancy Analysis System**: Analyzes job postings for consistency, identifies conflicts, and offers AI-powered improvement suggestions. Includes salary calculation transparency showing expected salary range with detailed breakdown (base salary by level, career track multiplier, geographic market tier adjustment, and certification premiums). Features HR-friendly "Why This Role Matches" explanations that translate NICE Framework competencies (Tasks, Knowledge, Skills) into plain language for non-technical HR professionals. Advanced anti-pattern detection includes: Frankenjob/Title Inflation (high-level titles paired with manual labor), Skill Stack Overflow (incompatible legacy + cutting-edge tech requirements), Burnout Risk/Single Point of Failure (24/7 on-call + zero-downtime + single headcount), and Salary Hierarchy Inversion (salary below market for stated qualifications).
