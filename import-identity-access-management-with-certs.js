...
async function importIdentityAccessManagementWithCerts() {
  console.log('Starting Identity and Access Management import with certifications...');

  try {
    // Read the Identity and Access Management track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_9_Identity_and_Access_Management_with_Certs.xlsx');
    ...
    // Find the Identity and Access Management track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Identity%' AND name ILIKE '%Access%' AND name ILIKE '%Management%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Identity and Access Management track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Identity and Access Management', 'Manage digital identities, authentication, authorization, and access controls across enterprise environments']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Identity and Access Management track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for IAM
      ...
      if (certName.includes('microsoft')) certMap.set('microsoft', cert.id);
      if (certName.includes('okta')) certMap.set('okta', cert.id);
    });
    ...
    console.log('\nIdentity and Access Management import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Identity and Access Management track:', error);
  } finally {
    await pool.end();
  }
}

importIdentityAccessManagementWithCerts();