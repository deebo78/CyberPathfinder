...
async function importTechnologyResearchToolDevelopmentWithCerts() {
  console.log('Starting Technology Research and Tool Development import with certifications...');

  try {
    // Read the Technology Research and Tool Development track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_15_Technology_Research_and_Tool_Development_with_Certs.xlsx');
    ...
    // Find the Technology Research and Tool Development track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Technology%' AND name ILIKE '%Research%' AND name ILIKE '%Tool%' AND name ILIKE '%Development%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Technology Research and Tool Development track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Technology Research and Tool Development', 'Research, design, and develop cybersecurity tools, technologies, and innovative solutions for security challenges']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Technology Research and Tool Development track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for technology research and development
    ...
    console.log('\nTechnology Research and Tool Development import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Technology Research and Tool Development track:', error);
  } finally {
    await pool.end();
  }
}

importTechnologyResearchToolDevelopmentWithCerts();