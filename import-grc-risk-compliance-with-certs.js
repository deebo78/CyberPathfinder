...
async function importGRCRiskComplianceWithCerts() {
  console.log('Starting GRC Risk Compliance import with certifications...');

  try {
    // Read the GRC track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_5_GRC_Risk_Compliance_with_Certs.xlsx');
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations
      ...
      if (certName.includes('iso 27001')) certMap.set('iso 27001', cert.id);
    ...
    console.log('\nGRC Risk Compliance import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing GRC Risk Compliance track:', error);
  } finally {
    await pool.end();
  }
}

importGRCRiskComplianceWithCerts();