...
async function importSecurityAutomationOrchestrationWithCerts() {
  console.log('Starting Security Automation and Orchestration import with certifications...');

  try {
    // Read the Security Automation and Orchestration track with certifications file
    const workbook = XLSX.readFile('attached_assets/Track_16_Security_Automation_and_Orchestration_with_Certs.xlsx');
    ...
    // Find the Security Automation and Orchestration track
    const trackResult = await pool.query(
      "SELECT id FROM career_tracks WHERE name ILIKE '%Security%' AND name ILIKE '%Automation%' AND name ILIKE '%Orchestration%'"
    );

    if (trackResult.rows.length === 0) {
      console.log('Security Automation and Orchestration track not found, creating it...');
      const insertResult = await pool.query(
        "INSERT INTO career_tracks (name, description) VALUES ($1, $2) RETURNING id",
        ['Security Automation and Orchestration', 'Design, implement, and manage automated security solutions and orchestration platforms for streamlined security operations']
      );
      var trackId = insertResult.rows[0].id;
    } else {
      var trackId = trackResult.rows[0].id;
      console.log(`Found Security Automation and Orchestration track with ID: ${trackId}`);
    }
    ...
    certResult.rows.forEach(cert => {
      ...
      // Add common certification variations for security automation
      ...
      if (certName.includes('python')) certMap.set('python', cert.id);
    });
    ...
    console.log('\nSecurity Automation and Orchestration import with certifications completed successfully!');

  } catch (error) {
    console.error('Error importing Security Automation and Orchestration track:', error);
  } finally {
    await pool.end();
  }
}

importSecurityAutomationOrchestrationWithCerts();